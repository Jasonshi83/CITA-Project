...
test_cli_js_beautify()
{
  ...
  unset HOME
  unset USERPROFILE
  $CLI_SCRIPT -o $TEST_TEMP/example1-calendar.js $SCRIPT_DIR/resources/example1.js || exit 1

  $CLI_SCRIPT -o $TEST_TEMP/example1-sanity.js $TEST_TEMP/example1-calendar.js || exit 1
  diff -q $TEST_TEMP/example1-calendar.js $TEST_TEMP/example1-sanity.js || {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be identical after no change in settings."
      cleanup 1
  }

  cd $SCRIPT_DIR/resources/configerror
  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js 2>&1 |  grep -q "Error while loading beautifier configuration\." || {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be configration load error message."
      cleanup 1
  }

  cd $SCRIPT_DIR/resources/indent11chars
  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js | diff -q $TEST_TEMP/example1-calendar.js - && {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be different based on CWD settings."
      cleanup 1
  }

  cd $SCRIPT_DIR/resources/indent11chars/subDir1/subDir2
  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js | diff -q $TEST_TEMP/example1-calendar.js - && {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be different based on CWD parent folder settings."
      cleanup 1
  }
  cd $SCRIPT_DIR
  ...
  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js | diff -q $TEST_TEMP/example1-calendar.js - && {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be different based on HOME settings."
      cleanup 1
  }

  $CLI_SCRIPT -o $TEST_TEMP/example1-indent11chars.js $TEST_TEMP/example1-calendar.js

  unset HOME
  export USERPROFILE=$SCRIPT_DIR/resources/indent11chars
  # node -p 'process.env["USERPROFILE"] || process.env["HOME"] || "unset"'
  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js | diff -q $TEST_TEMP/example1-indent11chars.js - || {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be identical for same HOME and USERPROFILE settings."
      cleanup 1
  }

  $CLI_SCRIPT $TEST_TEMP/example1-calendar.js | diff -q $TEST_TEMP/example1-calendar.js - && {
      echo "js-beautify output for $TEST_TEMP/example1-calendar.js was expected to be different based on USERPROFILE settings."
      cleanup 1
  }

  #meta-parameter brace_style
  $CLI_SCRIPT -b 'invalid' $TEST_TEMP/example1-calendar.js > /dev/null && {
    echo "[$CLI_SCRIPT_NAME -b 'invalid' $TEST_TEMP/example1-calendar.js] Return code for invalid brace_style meta-parameter should be error."
    cleanup 1
  }
  $CLI_SCRIPT -b 'expand,preserve-inline,invalid' $TEST_TEMP/example1-calendar.js > /dev/null && {
    echo "[$CLI_SCRIPT_NAME -b 'expand,preserve-inline,invalid' $TEST_TEMP/example1-calendar.js] Return code for invalid brace_style meta-parameter should be error."
    cleanup 1
  }
  $CLI_SCRIPT -b 'preserve-inline' $TEST_TEMP/example1-calendar.js > /dev/null || {
    echo "[$CLI_SCRIPT_NAME -b 'preserve-inline' $TEST_TEMP/example1-calendar.js] Return code for only one part of valid brace_style meta-parameter should be success (uses default where it can)."
    cleanup 1
  }
